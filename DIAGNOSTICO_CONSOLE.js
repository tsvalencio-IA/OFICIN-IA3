...
(function() {
  ...
  if (!window.J) { err('window.J não existe — jarvis.html não carregou corretamente'); }
    ...
    inf('IA Local key: '        + (window.J.IA Local ? '✓ configurada (' + window.J.IA Local.slice(0,8)+'...)' : '❌ NÃO configurada'));
  ...
  if (!gemKey) {
    err('Chave IA Local NÃO encontrada em J.IA Local');
    warn('Solução: no Firestore, acesse oficinas/{id_oficina} e adicione o campo cerebro com sua chave do Google AI Studio');
  } else {
